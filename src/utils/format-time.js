...
export const formatTimeWithRelativeDay = (timestamp) => {
  // Parse timestamp correctly
  const date = dayjs(Number(timestamp).toString() === timestamp ? Number(timestamp) : timestamp).locale('vi');
  ...
  // Check relative day and return the formatted string
  if (date.isToday()) {
...
export function formattedDate(timestampString) {
  return dayjs(parseInt(timestampString, 10)).format('YYYY-MM-DD HH:mm:ss');
}
...
export function formatDate(inputDate, formatTime = 'HH:mm:ss DD-MM-YYYY') {
  if (!inputDate) return '';

  return dayjs(inputDate).format(formatTime);
}